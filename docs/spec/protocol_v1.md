...
> **Purpose:** This document defines the authoritative wire contract (envelope shape, commands, errors, artifactization) for agentctl Protocol v1. This is the stable foundation that skills, plugins, jobs, agents, and all I/O must conform to.
...
- **Envelope shape**: One JSON structure for everything (skills, plugins, jobs, agents)
...
Every operation (skill execution, plugin call, job result, agent message) uses this JSON envelope:
...
```jsonc
{
  ...
  "meta": {
    ...
    "cas_digest": "sha256:...",       // Present when data.artifact exists
...
**Streaming**:
- Use **NDJSON**: zero or more `progress` events, followed by single terminal `ok|error` envelope
...
- `runner`: Execution environment (`wasi`, `exec`, `oci`, or `null` for built-ins)
...
- `profiles`: Array of active profiles (e.g., `["core/v1"]`, `["core/v1", "agent/v1"]`)
- `source`: How result was obtained (`run`, `cache`, `memory`)
- `cas_digest`: CAS digest when `data.artifact` present
...
**Rule**: If serialized `data.body` (or equivalent payload) would exceed `inline_output_kb` (default 32KB), replace inline content with a CAS artifact and emit a summary.
...
```jsonc
{
  ...
  "data": {
    ...
    "kind": "application/json",            // MIME type (redundant with summary)
    "size_bytes": 1048576                  // Size in bytes
  },
  "meta": {
    "cas_digest": "sha256:abc123...",      // MUST match data.artifact
    /* ... other meta fields ... */
  }
}
```
...
- **headers**: Curated headers (content-type, etag, rate-limit, caching, pagination, tracing)
...
Progress events provide bounded, machine-friendly updates during long-running operations.
