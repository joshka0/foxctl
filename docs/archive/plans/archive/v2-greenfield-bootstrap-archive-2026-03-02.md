...
**Approach**: Strangler Fig pattern — v2 lives in `internal/v2/` alongside v1. Command surfaces `spawn,ask,run,list,kill` are now hard-cut to v2 paths in CLI/API/daemon.

**Why**: Allows incremental migration with low risk to v1 stability. Each command can be validated independently before cutting over.

**Safety model**: No command-level `--dry-run` gate is required for v2 rollout. Safety comes from idempotency keys (`request_id`), append-only events, and controlled rollback.
...
Historical note: references below to command-level v1 fallback flags, shadow routing, `internal/v2/ports/*`, and `internal/v2/adapters/v1bridge/*` describe completed migration phases and are retained as archived implementation history.

**Dependency direction**:
```
command/api/daemon handlers → services → core
...
- Supported command surfaces are v2-only in active CLI/API/daemon paths.
...
| Adapter | `internal/v2/adapters/{libsql,mailbox,sourceimport}/*` | Keep storage/integration concerns behind explicit interfaces |
...
2. `TestV2Flags_Parse_ValidationErrors` — spacing, duplicates, invalid command rejection
...
## Historical Migration Archive (Pre-Hard-Cut)

The PR slices below are retained for audit/history. They reference transitional
routing/bridge files that were removed during hard-cut cleanup.
...
**Tests** (5):
1. `TestSpawnService_OutputContainsIDs` — spawn output and run/agent IDs
2. `TestSpawnService_DuplicateRequestIDIdempotent` — deduping behavior
3. `TestSpawnService_PolicyGate` — policy gate behavior
...
**Definition of done**: Side-by-side run capability in migration phase, smoke tests for spawn/list/ask/kill pass, and parity visibility across routes.
...
**Tests** (5):
1. `TestRouter_V1PrimaryWithV2Shadow` — v1 primary with v2 shadow
2. `TestDispatchWithShadow_DoesNotRunWhenCommandNotEnabled` — shadow allowlist
3. `TestRouter_NoDuplicateRunWhenV2Primary` — no duplicate run when v2 primary
...
1. Keep live command paths v2-only for supported command surfaces (`spawn`, `ask`, `run`, `list`, `kill`).
...
5. Complete decommission cleanup for command-surface routing artifacts and tests.

### Areas That Need Consideration and Fleshing Out

1. **Live routing integration**
   - Ensure CLI/API/daemon entrypoints dispatch directly to v2 services/adapters (no `internal/v2/ports/*` routers).
   - Keep rollback at deploy/process boundary (binary/config rollout), not per-command runtime routing flags.
...
## Wave 4: Identity-Guided Retrieval + Narrative Views (PR-25+)

Wave 4 extends v2 from artifact-level retrieval to hierarchical memory views that
remain anchored to immutable turn evidence.

### Wave 4 Goals

1. Add a semantic episode layer so retrieval can scale by chapter before turn-level drill-down.
2. Add a `WorkingContext` gate so retrieval eligibility is constrained by current task state.
3. Add a narrative artifact type that improves coherence while enforcing evidence citations.
4. Keep all Wave 4 components non-blocking and libsql-first for persistence/search.

### Consistency Rules (Locked for Wave 4)

1. **Immutable source**: turns/iterations/tool-calls remain append-only.
2. **Derived mutability only**: episodes and narrative are versioned derived artifacts.
3. **Evidence requirement**: derived claims must cite anchor refs.
4. **Async only**: episode/narrative compilers run in enricher/background pipelines and cannot block turn completion.
5. **Live/backfill parity**: source resynthesis and live pipelines must produce compatible schemas.

### PR-25 (M3): Episode Layer + Compiler Enricher

Scope:
1. Add `v2_episodes` with `session_id`, `start_turn_id`, `end_turn_id`, `topic`,
   `summary`, `salience_score`, `is_landmark`, and compiler version metadata.
2. Add episode compiler enricher triggered from `turn.recorded` and optional backfill flow.
3. Detect boundary/landmark cues with deterministic v1 rules:
   - boundary when any condition matches:
     - semantic shift `cosine_distance >= 0.30` against prior-turn embedding
     - todo phase completion (`todo_pending+todo_active` transitions from `>0` to `0`) with at least 3 turns since last boundary
     - explicit decision transition (`decision` anchor present) with at least 4 turns since last boundary
   - landmark when any condition matches:
     - episode contains at least 2 `decision` anchors
     - episode max salience `>= 0.70`
     - error-recovery pattern (`error` anchor in prior turns plus success marker in current boundary turn)
4. Add drill-down metadata linking episodes to referenced turn anchors.

Acceptance criteria:
1. Episode writes are idempotent by `(session_id, episode_version, boundary_key)`.
2. Retrieval can list episodes in deterministic order and resolve referenced turns.
3. Compiler failures emit events and do not fail completed turns.
4. Deterministic regression fixtures validate boundary/landmark outcomes for fixed turn corpora.

### PR-26 (M4): WorkingContext Gate on Retrieval

Scope:
1. Extend retrieval options with `WorkingContext`:
   - `session_id`
   - `workspace_id`
   - `active_files`
   - `required_labels`
   - `min_salience`
2. Apply hard SQL filters before vector ranking in libsql adapters.
3. Apply deterministic soft rerank combining similarity, salience, and label overlap.
4. Define deterministic graceful-fallback ladder when hard filters overconstrain:
   - level `0`: strict (all filters)
   - level `1`: relax `required_labels` only
   - level `2`: relax `min_salience` to `0.0`
   - level `3`: return temporal-only refs (no semantic hits)
5. Expose gate metadata in context-builder outputs and wide events:
   - `working_context_applied`
   - `working_context_fallback_level`
   - `working_context_eligible_count`

Acceptance criteria:
1. Retrieval eligibility reflects hard filters exactly (test-covered).
2. Rerank ordering is deterministic with stable tie-breakers.
3. Gate misses follow fallback levels `0 -> 3` exactly and emit matching metadata.
4. Fallback results remain bounded by request limit and deterministic ordering rules.

### PR-27 (M5): Narrative Artifact (Evidence-Cited)

Scope:
1. Add `narrative` as a v2 artifact type with session-scoped derived documents.
2. Require each narrative claim to include one or more anchor refs.
3. Add narrative compiler component with hybrid refresh policy:
   - event trigger: every 12 new turns in a session
   - time trigger: refresh when last narrative snapshot is older than 30 minutes
   - manual trigger: explicit resynthesis/maintenance invocation
4. Integrate narrative snippets into layered context assembly with explicit provenance metadata.

Acceptance criteria:
1. Uncited narrative claims are rejected by validation.
2. Narrative refresh is idempotent by `(session_id, artifact_type=narrative, artifact_version)`.
3. Context builder can include narrative blocks without breaking existing temporal/semantic ordering guarantees.
4. Narrative staleness metadata is emitted and must stay within policy bounds in tests.
