...
agentctl follows a layered architecture with explicit dependency direction to
...
**Purpose**: Core business logic with zero dependencies on other internal
packages.
...
Layer violations are automatically detected via `golangci-lint` with the
`depguard` linter.
...
This layered structure was established through **SPEC-008: Reorganize Internal
Packages** to improve:
