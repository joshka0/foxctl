...
func cancelJobCmd(jobID string) tea.Cmd {
	return func() tea.Msg {
		...
		cmd := exec.CommandContext(ctx, "foxctl", "jobs", "cancel", jobID)
...
func completeTaskCmd(workspace, taskID string) tea.Cmd {
	return func() tea.Msg {
		...
		cmd := exec.CommandContext(ctx, "foxctl", "run", "todo/manage", "--input", string(input))
		output, err := cmd.CombinedOutput()
		if err != nil {
...
func ackMessageCmd(workspace, actorID, messageID string) tea.Cmd {
	return func() tea.Msg {
		...
		cmd := exec.CommandContext(ctx, "foxctl", "run", "mailbox/manage", "--input", string(input))
		output, err := cmd.CombinedOutput()
		if err != nil {
...
func fetchSearchResults(query string, limit int, rerank bool, scopes []string) ([]searchResult, *searchStats, error) {
	...
	cmd := exec.CommandContext(ctx, "foxctl", "run", "code/semantic_search", "--input", string(inputJSON))
	output, err := cmd.CombinedOutput()
	if err != nil {
