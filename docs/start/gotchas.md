...
| R3      | G3        | CGO builds/tests must use `-tags=libsqlite3`               | CI must run at least one `CGO_ENABLED=1` build/test with `-tags=libsqlite3`; failures often show `duplicate symbol '_sqlite3_*'`.                | Requires system SQLite dev headers/libs (CI: `libsqlite3-dev`; macOS: Homebrew `sqlite`).                                                                               |
