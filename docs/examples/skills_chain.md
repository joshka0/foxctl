...
agentctl skills run fs/ls --input '{"path":"."}' \
  ...
  | agentctl skills run fs/read --input-file - \
  | jq '{path: .data.path, pattern: "needle"}' \
  | agentctl skills run text/grep --input-file -
```

`fs/read` streams the full file into CAS (so large files stay out of band) while returning a UTF-8 preview inline. The digest it emits can be fed into `agentctl cas get` or attached to higher-level envelopes when you need downstream reproducibility. When you do not need the intermediate `jq` plumbing, call it directly as `agentctl fs read path/to/file` and let the CLI craft the JSON envelope for you.

## TODO capture with `todo/manage`

You can log follow-up work straight from a bash pipeline by piping structured JSON into the new todo skill (or simply call `agentctl todo add` for flags-first ergonomics):

```bash
cat <<'JSON' \
  | agentctl skills run todo/manage --input-file -
{
  "operation": "add",
  "add": {
...
```bash
agentctl skills run todo/manage --input '{
  "operation":"complete",
