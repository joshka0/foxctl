# LEANN vs Memvid vs agentctl: Vector Storage Architecture Comparison
...
| **agentctl** | Scope-based flexibility | Multi-database + pluggable providers | Live operations, cross-workspace |

**Key Finding**: LEANN's selective recomputation approach could dramatically reduce agentctl's storage footprint (97% savings) while maintaining search quality. This is particularly relevant for:
...
## 3. agentctl: Multi-Database + Pluggable Providers
...
| Scenario | agentctl | Memvid | LEANN |
|----------|----------|--------|-------|
...
**LEANN advantage**: 97% storage reduction vs agentctl

### Search Quality

| Metric | agentctl (Voyage 1024d) | Memvid (384d) | LEANN (any dims) |
|--------|-------------------------|---------------|------------------|
...
| Scenario | agentctl | Memvid | LEANN HNSW | LEANN DiskANN |
|----------|----------|--------|------------|---------------|
...
| Capability | agentctl | Memvid | LEANN |
|------------|----------|--------|-------|
...
### Strategy C: Hybrid agentctl + Memvid + LEANN
...
- [agentctl Progressive Memory](../designs/progressive-memory-system.md)
