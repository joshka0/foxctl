...
CGO_ENABLED=1 go build -o bin/foxctl-cgo ./cmd/foxctl  # ✅ Success

# Without CGO (default, portable)
CGO_ENABLED=0 go build -o bin/foxctl ./cmd/foxctl     # ✅ Success
```

### 3. Build Metadata Verification
```bash
$ go version -m foxctl-cgo | grep CGO
build   CGO_ENABLED=1        # ✅ Correct

$ go version -m foxctl-nocgo | grep CGO
...
1. **Core foxctl:** Continue building with `CGO_ENABLED=0` (portable)
