...
This document lists the core **code areas** in `agentctl` that should have
Codemaps.\
Each entry has:
...
- **Prompt**:\
  “Map the complete envelope pipeline in agentctl, starting from Cobra command
  handlers under `cmd/agentctl/cmd/` through the domain `envelope` package. Show
  ...
  `cmd/agentctl/cmd/*.go` call into them, and where RFC3339 timestamps and
...
- **Name**:
  `@[agentctl Skill System: Installation, Execution, Jobs, CAS, and Plugin Architecture]`
...
- **Prompt**:\
  “Map the job system in agentctl, from job creation to completion. Start at the
...
- **Name**:
  `@[agentctl Overseer & Agent Hierarchy: Spawn, Mailbox, Depth Limits]`
...
- **Prompt**:\
  “Produce an end-to-end Core Profile v1 codemap for agentctl. Starting from a
