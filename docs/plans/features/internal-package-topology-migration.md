...
| `internal/runtime/execution/agentmanager` | `internal/v2/services/{spawn,kill,list,run}` | fallback path remains in places |
...
- scope the **history** tranche as one bounded workflow:
  - `internal/context/transcriptpipeline` produces transcript-derived history artifacts
    and packs
  - `internal/context/contextplane/taskhistory` consumes those outputs for
...
| `internal/intelligence/searchquery` | search/query/recall | keep as query-planning slice |
...
- do not move raw snippet extraction into `internal/intelligence/codemap`
...
- moving `internal/storage/*`, `internal/context/contextplane`, or intelligence families
  under `internal/v2`
