...
- [x] PR-11: Live command-surface cutover (v2 routing in real CLI/API/daemon handlers)
  ...
  - [x] wire API agent spawn/daemon action handlers to v2 command handlers in `internal/web/api/agents.go`
...
- [x] PR-21: command-surface de-legacy dedup (remove no-op v2 passthrough wrappers)
  ...
  - [x] remove no-op `*V2` wrappers in `cmd/agentctl/cmd/agent.go` and `internal/web/api/agents.go`
...
- 2026-03-02: Completed hard-cut cleanup for command routing migration artifacts.
  ...
  - Validation: `go test ./internal/v2/... ./internal/daemon ./internal/web/api ./cmd/agentctl/cmd` and `make check-doc-links` (pass).
- 2026-02-27:
  Subagent Review
...
- 2026-02-27:
  ...
  - scope: `PR-30 gui-agent route audit + event/context alignment` (`internal/web/api/agents.go`, `internal/observability/{wide_event.go,sse_bridge.go,sse_bridge_test.go}`, `internal/v2/runtime/contextbuilder/{layered.go,layered_internal_test.go,layered_observability_test.go}`, `packages/gui-agent/src/api/types.ts`)
  - findings: `none`
  - decision: `approved`
- 2026-02-27:
  Subagent Review
  - reviewer: `019c9f17-3de5-7ac1-8093-1bcf4faf3022`
  - scope: `PR-30 gui-agent route audit + event/context alignment` (`internal/web/api/agents.go`, `internal/observability/{wide_event.go,sse_bridge.go,sse_bridge_test.go}`, `internal/v2/runtime/contextbuilder/{layered.go,layered_observability_test.go}`, `packages/gui-agent/src/api/types.ts`)
...
- 2026-02-27: Completed PR-30 gui-agent route audit + v2 contract alignment.
  ...
  - Validation: `CGO_ENABLED=0 go test ./internal/observability ./internal/v2/runtime/contextbuilder ./internal/web/api -count=1` and `pnpm -C packages/gui-agent exec tsc --noEmit` (pass).
- 2026-02-27:
  Subagent Review
...
- 2026-02-22:
  ...
  - scope: `PR-24 explicit v2 spawn path` (`cmd/agentctl/cmd/agent.go`, `internal/web/api/agents.go`, `internal/daemon/service.go`, `docs/plans/v2-implementation-todo.md`)
...
- 2026-02-22: Completed PR-24 explicit v2 spawn command path.
  ...
  - Verification: `go test ./cmd/agentctl/cmd ./internal/web/api ./internal/daemon ./internal/v2/...` (pass).
- 2026-02-22:
  Subagent Review
  ...
  - scope: `PR-23 explicit v2 kill path` (`internal/daemon/service.go`, `internal/web/api/agents.go`, `cmd/agentctl/cmd/agent.go`, `docs/plans/v2-implementation-todo.md`)
  - findings: `none`
  - decision: `approved`
...
- 2026-02-22:
  ...
  - scope: `PR-22 explicit v2 list path` (`cmd/agentctl/cmd/agent.go`, `internal/web/api/agents.go`, `internal/daemon/service.go`, `docs/plans/v2-implementation-todo.md`)
...
- 2026-02-22: Completed PR-22 explicit v2 list command path.
  ...
  - Verification: `go test ./cmd/agentctl/cmd ./internal/web/api ./internal/daemon` and `go test ./internal/v2/...` (pass).
- 2026-02-21:
  Subagent Review
  - reviewer: `019c829e-2f38-7e31-9e78-b22e43ed600f`, `019c829e-305d-7e41-90f5-acdb7875d24b`
  - scope: `PR-21 command-surface de-legacy dedup` (`cmd/agentctl/cmd/agent.go`, `internal/web/api/agents.go`, `internal/daemon/service.go`, targeted routing tests)
