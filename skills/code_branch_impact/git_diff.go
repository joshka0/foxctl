...
func (p gitDiffProvider) ChangedFiles(ctx context.Context, in branchimpact.Input) ([]branchimpact.Change, error) {
...
func diffRangeArgs(in branchimpact.Input) []string {
	base := strings.TrimSpace(in.BaseRef)
	if base == "" {
		base = branchimpact.DefaultBaseRef
	}
...
func parseNameStatus(raw string) []branchimpact.Change {
	fields := splitNUL(raw)
	changes := make([]branchimpact.Change, 0, len(fields)/2)
	for i := 0; i < len(fields); {
		...
		change := branchimpact.Change{Status: status}
