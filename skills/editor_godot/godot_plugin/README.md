...
This Godot 4 EditorPlugin enables the `editor/godot` agentctl skill to interact
...
Once the plugin is running, you can use agentctl to interact with the editor:
...
# Get scene tree
agentctl run editor/godot --input '{"action": "scene_tree"}'

# Inspect a node
agentctl run editor/godot --input '{"action": "node_inspect", "node_path": "/root/Main/Player"}'

# Create a node
agentctl run editor/godot --input '{"action": "node_create", "parent_path": "/root/Main", "node_type": "Node2D", "node_name": "Enemy"}'

# Set a property
agentctl run editor/godot --input '{"action": "node_set_prop", "node_path": "/root/Main/Player", "property": "position", "value": "Vector2(100, 200)"}'
```
...
- Workspace validation ensures the agentctl workspace matches the Godot project.
...
Run agentctl from your Godot project directory, or use `--workspace` to specify
the correct path.
