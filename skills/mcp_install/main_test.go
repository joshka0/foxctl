...
func TestCommand(t *testing.T) {
	assert.Equal(t, "mcp/install", command)
}
...
func TestInput_AllFields(t *testing.T) {
	in := input{
		ServerCmd:     "npx",
		ServerArgs:    []string{"-y", "@modelcontextprotocol/server-sqlite"},
		ServerEnv:     map[string]string{"NODE_ENV": "production"},
		ServerURL:     "http://localhost:8080/sse",
		ServerHeaders: map[string]string{"Authorization": "Bearer token"},
		OutputDir:     "/path/to/output",
		BridgePath:    "/usr/local/bin/mcp_bridge",
	}

	assert.Equal(t, "npx", in.ServerCmd)
	assert.Equal(t, []string{"-y", "@modelcontextprotocol/server-sqlite"}, in.ServerArgs)
	...
	assert.Equal(t, "/path/to/output", in.OutputDir)
	assert.Equal(t, "/usr/local/bin/mcp_bridge", in.BridgePath)
}
...
func TestInput_JSONFieldNames(t *testing.T) {
	in := input{
		...
		ServerEnv:     map[string]string{"K": "V"},
		ServerURL:     "url",
	...
	assert.Contains(t, jsonStr, "output_dir")
	assert.Contains(t, jsonStr, "bridge_path")
}
