...
func NewEngine(index SearchIndex, embedder Embedder) *Engine {
	return &Engine{index: index, embedder: embedder}
}

// WithRepoQueryService attaches an optional structural-recall service.
func (e *Engine) WithRepoQueryService(service RepoQueryService) *Engine {
	if e == nil {
		return nil
	}
	e.repo = service
...
func (e *Engine) embedQuery(ctx context.Context, query string, mode QueryEmbeddingMode) ([]float32, error) {
	...
	case QueryEmbeddingModeEmbedQuery:
		if qp, ok := e.embedder.(QueryEmbedder); ok {
