// Package profiles owns built-in ATCP adapter profile defaults.
//
...
func mustLoadBuiltins(raw []byte) map[string]profileFileEntry {
	...
	if err := json.Unmarshal(raw, &entries); err != nil {
		panic("atcp adapter profiles: invalid built-in profiles: " + err.Error())
	}
	out := make(map[string]profileFileEntry, len(entries))
	for _, entry := range entries {
		name := normalize(entry.Name)
		if name == "" {
			panic("atcp adapter profiles: built-in profile has empty name")
		}
		entry.Name = name
		out[name] = entry
