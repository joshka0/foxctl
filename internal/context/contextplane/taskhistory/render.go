...
func RenderHookContextWithArtifact(pack Pack, artifactDigest string) string {
	...
	if len(pack.ContextWikiNotes) > 0 {
		b.WriteString("\n\n**ContextWiki notes:** ")
		notes := make([]string, 0, minInt(3, len(pack.ContextWikiNotes)))
		for _, hit := range pack.ContextWikiNotes[:minInt(3, len(pack.ContextWikiNotes))] {
...
func RenderJidoStateWithArtifact(pack Pack, artifactDigest string) map[string]any {
	state := map[string]any{
		...
		"contextwiki_note_cnt": len(pack.ContextWikiNotes),
