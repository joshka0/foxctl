...
// NewEinoToolBridge creates a new Eino tool from an agentctl ToolDef and ToolExecutor.
...
// NewEinoToolBridges creates a slice of Eino tools from agentctl ToolDefs and a single ToolExecutor.
...
// Info implements tool.BaseTool by mapping agentctl ToolDef to Eino schema.ToolInfo.
...
// InvokableRun implements tool.InvokableTool by delegating to the agentctl ToolExecutor.
func (b *einoToolBridge) InvokableRun(ctx context.Context, argumentsInJSON string, _ ...tool.Option) (string, error) {
	...
	// agentctl ToolExecutor.Execute expects json.RawMessage.
