...
// TursoStore implements CASStore using Turso as the backend.
...
// NewTursoStore creates a new Turso-backed CAS store.
func NewTursoStore(ctx context.Context, cfg TursoConfig) (*TursoStore, error) {
	threshold := cfg.BlobThreshold
	if threshold == 0 {
		threshold = 10 << 20 // Default 10MB for Turso
	}
	...
	db, err := dbdriver.OpenDB(ctx, dbCfg, migrateTursoSchema)
	if err != nil {
		return nil, fmt.Errorf("cas: open turso: %w", err)
	}
...
// migrateTursoSchema creates the CAS tables in Turso.
func migrateTursoSchema(ctx context.Context, db *sql.DB) error {
	schema := `
...
func (s *TursoStore) Put(ctx context.Context, r io.Reader, kind string, tags []string) (Object, error) {
	if kind == "" {
...
func (s *TursoStore) mergeTagsAndReturn(ctx context.Context, digest string, newTags []string) (Object, error) {
...
func (s *TursoStore) Get(ctx context.Context, digest string) (io.ReadCloser, Metadata, error) {
...
func (s *TursoStore) Head(ctx context.Context, digest string) (Object, error) {
	if !digestPattern.MatchString(digest) {
...
func (s *TursoStore) List(ctx context.Context) ([]Object, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *TursoStore) Remove(ctx context.Context, digest string) error {
	if !digestPattern.MatchString(digest) {
...
func (s *TursoStore) Pin(ctx context.Context, digest string) error {
	if !digestPattern.MatchString(digest) {
		return fmt.Errorf("cas: invalid digest")
	}
...
func (s *TursoStore) Unpin(ctx context.Context, digest string) error {
	if !digestPattern.MatchString(digest) {
		return fmt.Errorf("cas: invalid digest")
	}
...
func (s *TursoStore) AddTags(ctx context.Context, digest string, tags []string) error {
	if !digestPattern.MatchString(digest) {
		return fmt.Errorf("cas: invalid digest")
	}
...
func (s *TursoStore) GC(ctx context.Context, opts GCOptions) (GCResult, error) {
