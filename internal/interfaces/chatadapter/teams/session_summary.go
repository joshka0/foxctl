...
func (a *Adapter) fetchSessionSummary(ctx context.Context, sessionID string) (sessionSummary, error) {
	...
	if resp.StatusCode >= 400 {
		return sessionSummary{}, fmt.Errorf("fetch session failed (HTTP %d): %s", resp.StatusCode, truncateForTeams(string(raw)))
	}
...
func (a *Adapter) postAgentSummary(parent context.Context, agentID string, state agentThread) {
	if a.cfg.AgentConversationID == "" || state.RootActivityID == "" {
		return
	}
	...
	go func() {
		...
		if err != nil {
			observability.Emit(context.Background(),
				observability.NewEvent("teams.session_summary_failed").
					WithComponent("teams").
		...
		card := sessionSummaryCard(agentID, sess)
		actID, err := a.replyInConversation(ctx, a.cfg.AgentConversationID, state.RootActivityID, card)
		if err == nil && actID != "" {
			convKey := a.cfg.AgentConversationID
			a.agentRootIdx.Store(agentRootKey(convKey, actID), agentRootIndexEntry{AgentID: agentID, LastSeen: a.clock.Now()})
		}
	}()
}
