...
func searchBacktrackPresetSource() string {
    ...
    Input fields:
      prompt: str -- the problem description with node_0, node_1, ... sub-problems
      answer_format: str -- expected output format (optional)

    Output: solution = {"node_0": <answer>, "node_1": <answer>, ...}

    Algorithm:
    1. Parse the problem into numbered sub-problems (node_0, node_1, ...).
    2. Identify dependencies: "For this value use ... from problem node_N".
    3. Solve leaf nodes first (no dependencies), then propagate forward.
    4. Return a JSON dict mapping node names to their answers.

    Each node answer should be:
    - A number (int or float)
    - A reduced fraction as a string "p/q"
    - An expression like "2013^{4025}"

    If you cannot solve a node, set its value to null.
    """
    import re
    prompt = input.get("prompt", "")
    ...
    # Parse nodes
    nodes = {}
    for m in re.finditer(r'Problem\s+(node_\d+)\s*:\s*(.+?)(?=Problem\s+node_\d+|$)', prompt, re.DOTALL):
        node_id = m.group(1)
        problem_text = m.group(2).strip()
        # Find dependencies
        deps = re.findall(r'from problem (node_\d+)', problem_text)
        nodes[node_id] = {"text": problem_text, "deps": deps, "answer": None}

    if not nodes:
        # Single problem
        nodes["node_0"] = {"text": prompt, "deps": [], "answer": None}

    # Solve in dependency order (topological)
    solved = set()
    for _ in range(len(nodes) * 2):
        ...
        for nid, ndata in nodes.items():
            if nid in solved:
                continue
            if all(d in solved for d in ndata["deps"]):
                # Substitute solved values into problem text
                text = ndata["text"]
                for dep in ndata["deps"]:
                    dep_ans = nodes[dep]["answer"]
                    if dep_ans is not None:
                        text = text.replace(f"[For this value use", f"[SUB:{dep}={dep_ans}")
                ndata["text"] = text
                solved.add(nid)
                progress = True
        if not progress:
    ...
    for nid in sorted(nodes.keys()):
        ans = nodes[nid]["answer"]
        solution[nid] = ans if ans is not None else "UNSOLVED"
