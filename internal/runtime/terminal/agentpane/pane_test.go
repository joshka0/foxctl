...
func TestCreateWatchPaneTmux(t *testing.T) {
	...
	runner := &tmuxSequenceRunner{steps: []tmuxSequenceStep{
		...
		{key: "tmux respawn-pane -k -t %71 -c /repo env FOXCTL_PARTICIPANT_ID=tmux:collab:%71 FOXCTL_MUX_BACKEND=tmux FOXCTL_MUX_SESSION=collab FOXCTL_MUX_PANE_ID=%71 FOXCTL_PARENT_PARTICIPANT_ID=parent-a FOXCTL_PARENT_AGENT_ID=agent:parent-1 foxctl agent watch agent-123"},
