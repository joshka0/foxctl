...
func TestDefaultSocketPath(t *testing.T) {
	...
	if !strings.HasSuffix(got, filepath.Join("foxctl-pane", "room_alpha", "claude-a.sock")) {
...
func TestDefaultSocketPathPrefersShortRootWhenTmpDirIsLong(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		if !strings.Contains(got, "foxctl-pane") {
			t.Fatalf("DefaultSocketPath() = %q, want foxctl-pane path", got)
		}
		return
	}
	if !strings.HasPrefix(got, filepath.Join(string(filepath.Separator), "tmp", "foxctl-pane")+string(filepath.Separator)) {
