...
func TestPrepareSessionRespawnsExistingShellPanesForAgent(t *testing.T) {
	...
	runner := &sequenceRunner{
		steps: []sequenceStep{
			...
			{key: "tmux set-option -p -t %21 @foxctl_participant codex-a"},
			{key: "tmux set-option -p -t %21 @foxctl_provider codex"},
			{key: "tmux set-option -p -t %21 @foxctl_room_id"},
			{key: "tmux set-option -p -t %21 @foxctl_wrapped 1"},
			{key: "tmux set-option -p -t %22 @foxctl_participant codex-b"},
			{key: "tmux set-option -p -t %22 @foxctl_provider codex"},
			{key: "tmux set-option -p -t %22 @foxctl_room_id"},
			{key: "tmux set-option -p -t %22 @foxctl_wrapped 1"},
			{key: "tmux list-panes -t foxctl-agent-smoke -F " + listFormat, stdout: strings.Join([]string{
...
func TestPrepareSessionInjectsHierarchyEnvForRespawnedPanes(t *testing.T) {
	...
	runner := &sequenceRunner{
		steps: []sequenceStep{
			...
			{key: "tmux set-option -p -t %31 @foxctl_participant codex-a"},
			{key: "tmux set-option -p -t %31 @foxctl_provider codex"},
			{key: "tmux set-option -p -t %31 @foxctl_room_id"},
			{key: "tmux set-option -p -t %31 @foxctl_wrapped 1"},
...
func TestPrepareSessionInjectsDirectRoomEnvForTopLevelPanes(t *testing.T) {
	...
	runner := &sequenceRunner{
		steps: []sequenceStep{
			...
			{key: "tmux set-option -p -t %41 @foxctl_participant codex-a"},
			{key: "tmux set-option -p -t %41 @foxctl_provider codex"},
			{key: "tmux set-option -p -t %41 @foxctl_room_id room-alpha"},
			{key: "tmux set-option -p -t %41 @foxctl_wrapped 1"},
...
func TestCreatePaneAllocatesAndRespawnsExactPane(t *testing.T) {
	...
	runner := &sequenceRunner{
		steps: []sequenceStep{
			...
			{key: "tmux respawn-pane -k -t %51 -c /repo env FOXCTL_PARTICIPANT_ID=tmux:pane-smoke:%51 FOXCTL_MUX_BACKEND=tmux FOXCTL_MUX_SESSION=pane-smoke FOXCTL_MUX_PANE_ID=%51 FOXCTL_PARENT_PARTICIPANT_ID=parent-a FOXCTL_PARENT_AGENT_ID=agent:parent-1 watch-cmd"},
...
func TestCreatePaneSetsViewerMetadataForWrappedProviderPane(t *testing.T) {
	...
	runner := &sequenceRunner{
		steps: []sequenceStep{
			...
			{key: "tmux respawn-pane -k -t %71 -c /repo env FOXCTL_PARTICIPANT_ID=claude-a FOXCTL_MUX_BACKEND=tmux FOXCTL_MUX_SESSION=pane-meta FOXCTL_MUX_PANE_ID=%71 FOXCTL_ROOM_ID=room-alpha " + command},
			...
			{key: "tmux set-option -p -t %71 @foxctl_participant claude-a"},
			{key: "tmux set-option -p -t %71 @foxctl_provider claude"},
			{key: "tmux set-option -p -t %71 @foxctl_room_id room-alpha"},
			{key: "tmux set-option -p -t %71 @foxctl_wrapped 1"},
...
func TestRespawnPaneReplacesExactTarget(t *testing.T) {
	runner := &sequenceRunner{
		steps: []sequenceStep{
			...
			{key: "tmux respawn-pane -k -t %61 -c /repo env FOXCTL_PARTICIPANT_ID=tmux:collab:%61 FOXCTL_MUX_BACKEND=tmux FOXCTL_MUX_SESSION=collab FOXCTL_MUX_PANE_ID=%61 FOXCTL_PARENT_PARTICIPANT_ID=parent-a FOXCTL_PARENT_AGENT_ID=agent:parent-1 watch-cmd"},
