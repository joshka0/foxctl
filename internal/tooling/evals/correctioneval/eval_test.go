...
func TestSummarize(t *testing.T) {
	summaries := Summarize([]CaseResult{
		{Method: "contextwiki_retrieve", ClassificationMatch: true},
		{Method: "contextwiki_retrieve", ClassificationMatch: false, FixChecked: true, FixMatch: true},
		{Method: "repoindex_dag", ClassificationMatch: true, FixChecked: true, FixMatch: false},
	})
	if len(summaries) != 2 {
		t.Fatalf("summaries=%d want 2", len(summaries))
	}
	if summaries[0].Method != "contextwiki_retrieve" || summaries[0].ClassificationAccuracy != 0.5 || summaries[0].FixAccuracy != 1.0 || summaries[0].FixChecks != 1 {
