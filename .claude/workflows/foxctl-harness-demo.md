# foxctl as AI Agent Harness - Demo Workflow

This document demonstrates how foxctl serves as a coordination harness for Claude and Gemini.
...
### Step 2: Gather Structured Context (foxctl)
...
foxctl run code/complexity --input '{"path": "."}'
```
...
foxctl mailbox send gemini-queries --from claude --payload '{"query": "..."}'

# After getting response
foxctl mailbox send claude-queries --from gemini --payload '{"response": "..."}'
```
...
```
Claude → foxctl skill → structured result
```
...
```
Claude → foxctl context → Gemini → response
```
...
2. **Use foxctl skills for structured data** - Faster than Gemini's file reads
3. **Log important queries to mailbox** - Creates audit trail
4. **Cache expensive operations** - foxctl auto-caches by default
...
```bash
# 1. Create task
foxctl todo add --title "Integration test"

# 2. Run skill
foxctl run code/complexity --input '{"path": ".", "threshold": 20}'

# 3. Check mailbox
foxctl mailbox list test-agent
