// foxprox-smoke spawns an agent CLI under the Foxprox broker, injects a prompt as a
// TerminalSubmit intent, and prints the streamed PTY output.
//
// Usage:
//
//	foxprox-smoke --agent droid   --prompt "say hi in one sentence"
//	foxprox-smoke --agent codex   --prompt "explain bubble sort in 1 line"
//	foxprox-smoke --agent gemini  --prompt "what is 2+2"
//	foxprox-smoke --agent claude  --prompt "greet me"
//	foxprox-smoke --cmd "bash"    --prompt "echo hello world"
//
...
import (
	...
	"github.com/joshka/foxprox/foxprox/broker/safeprompt"
...
func main() {
	...
	fmt.Fprintf(os.Stderr, "foxprox-smoke: spawning %v\n", cmd)
	...
	fmt.Fprintf(os.Stderr, "foxprox-smoke: session %s pid=%d\n", snap.ID, snap.PID)
	if *waitReady {
		...
		fmt.Fprintf(os.Stderr, "foxprox-smoke: waiting for safe prompt (idle=%v, timeout=%v)\n", *idleWindow, *readyTimeout)
		d, reason, err := safeprompt.Wait(readyCtx, sess, safeOpts, 75*time.Millisecond)
		cancelReady()
		if err != nil {
			if *unsafeSubmit {
				fmt.Fprintf(os.Stderr, "foxprox-smoke: WARN not ready (%s: %s); --unsafe-submit-anyway is set, injecting anyway\n", d, reason)
			} else {
				fatalf("not ready (%s: %s); rerun with --unsafe-submit-anyway to force", d, reason)
			}
		} else {
			fmt.Fprintf(os.Stderr, "foxprox-smoke: ready (%s)\n", reason)
		}
	} else {
		select {
	...
	fmt.Fprintf(os.Stderr, "foxprox-smoke: submitting prompt: %q (lease=%s)\n", *prompt, leaseRec.ID)
	if _, err := b.Submit(snap.ID, intents.TerminalSubmit{Text: *prompt, LeaseID: leaseRec.ID}); err != nil {
		fatalf("submit: %v", err)
	}

	fmt.Fprintf(os.Stderr, "foxprox-smoke: streaming output for %v (Ctrl-C to stop early)\n", *wait)
	stream(ctx, sess, startSeq, *wait)

	fmt.Fprintf(os.Stderr, "\nfoxprox-smoke: stopping session\n")
	if logSink != nil {
...
func fatalf(format string, args ...any) {
	fmt.Fprintf(os.Stderr, "foxprox-smoke: "+format+"\n", args...)
	os.Exit(1)
}
