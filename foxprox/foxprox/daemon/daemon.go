// Package daemon composes the Foxprox broker with its HTTP/JSON transport and a
...
//
// Persistence (foxprox_sessions, foxprox_rooms, foxprox_room_members) is a follow-up;
...
type Options struct {
	...
	// DataDir is the directory where the daemon opens `foxprox.db`. Empty
	// means fully in-memory operation. When non-empty, the daemon opens
	// SQLite at DataDir/foxprox.db and owns its lifecycle.
	DataDir string
...
// is the single source of truth for every Foxprox tool (daemon, client, CLI).
...
func (d *Daemon) Start() error {
	...
	defer d.mu.Unlock()
	if d.started {
		return errors.New("foxprox daemon: already started")
	}
	...
	if d.opts.DataDir != "" && brokerOpts.Storage == nil {
		if err := os.MkdirAll(d.opts.DataDir, 0o700); err != nil {
			return fmt.Errorf("foxprox daemon: data dir: %w", err)
		}
		dsn := "file:" + filepath.Join(d.opts.DataDir, "foxprox.db")
		s, err := sqlite.Open(context.Background(), dsn)
		if err != nil {
			return fmt.Errorf("foxprox daemon: sqlite open: %w", err)
		}
		owned = s
	...
	if err != nil {
		...
		return fmt.Errorf("foxprox daemon: broker: %w", err)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("foxprox daemon: listen: %w", err)
	}
	d.broker = b
	d.listener = ls
	d.ownedStore = owned
...
func (d *Daemon) Shutdown(ctx context.Context) error {
	...
	if owned != nil {
		if cerr := owned.Close(); cerr != nil && err == nil {
			err = fmt.Errorf("foxprox daemon: close store: %w", cerr)
		}
	}
	d.logger.Info("foxprox daemon shut down", slog.String("socket", d.opts.SocketPath))
	return err
}
