// Package broker composes the Foxprox subsystems — session manager, lease
...
import (
	...
	"github.com/joshka/foxprox/foxprox/adapter/generictty"
	"github.com/joshka/foxprox/foxprox/adapter/profiles"
...
// Broker is the single entrypoint for Foxprox intents originating from transport
...
func New(opts Options) (*Broker, error) {
	...
	if err := b.hydrate(ctx); err != nil {
		...
		b.sessions.Stop()
		b.leases.Stop()
		return nil, fmt.Errorf("foxprox broker: hydrate: %w", err)
	}
	return b, nil
}
...
// deliberately volatile in this slice; persistence (foxprox_rooms /
// foxprox_room_members) lands with the daemon wiring.
...
func (b *Broker) CreateRoom(ctx context.Context, req room.CreateRoomRequest) (room.Room, error) {
	...
	if perr := b.store.SaveRoom(ctx, r); perr != nil {
		return r, fmt.Errorf("foxprox broker: persist room: %w", perr)
	}
	return r, nil
}
...
func (b *Broker) JoinRoom(ctx context.Context, req room.JoinRequest) (room.Member, error) {
	...
	if perr := b.store.SaveMember(ctx, mem); perr != nil {
		return mem, fmt.Errorf("foxprox broker: persist member: %w", perr)
	}
	return mem, nil
}
...
func (b *Broker) LeaveRoom(ctx context.Context, roomID, agentID string) (room.Member, error) {
	...
	if perr := b.store.SaveMember(ctx, mem); perr != nil {
		return mem, fmt.Errorf("foxprox broker: persist member leave: %w", perr)
	}
	return mem, nil
}
...
func (b *Broker) SendMessage(ctx context.Context, msg router.Message) (router.Result, error) {
	...
	if perr := b.store.AppendMessage(ctx, rec); perr != nil {
		return res, fmt.Errorf("foxprox broker: persist message: %w", perr)
	}
	return res, nil
}
...
var (
	ErrSessionNotFound = errors.New("foxprox broker: session not found")
	ErrLeaseRequired   = errors.New("foxprox broker: terminal intents require an active lease on the session")
	ErrLeaseMismatch   = errors.New("foxprox broker: supplied lease_id does not match the current holder")
	ErrIntentInvalid   = errors.New("foxprox broker: intent invalid")
)
...
func (b *Broker) SessionReady(sessionID string) (bool, string, error) {
	...
	case ready.AltScreen:
		return false, "alt_screen", nil
	case !ready.Idle:
