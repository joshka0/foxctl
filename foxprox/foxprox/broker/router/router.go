// Package router fans out Foxprox message.send intents over the active members
// of a room.
//
...
type Message struct {
	...
	SkipAgents []string
	// ReceiptVisibility controls whether terminal recipients see an Foxprox
...
var (
	ErrRoomIDRequired      = errors.New("foxprox router: room_id is required")
	ErrEmptyMessage        = errors.New("foxprox router: message text is empty")
	ErrUnsupportedDelivery = errors.New("foxprox router: delivery policy not supported in this slice")
	ErrUnsafePrompt        = errors.New("foxprox router: terminal is not at a safe prompt")
	...
	ErrNoActiveMembers = errors.New("foxprox router: no active members in room")
	...
	ErrNoMutableMembers = errors.New("foxprox router: no mutable members in room")
)
...
func TerminalText(msg Message) string {
	...
	return strings.Join([]string{
		"[Foxprox receipt] " + encoded,
		"[Foxprox reply] print reply_prefix_hint with <AT> replaced by the at-sign character, then append your message.",
		"",
		msg.Text,
	}, "\n")
}
