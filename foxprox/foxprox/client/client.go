// Package client is a thin HTTP/JSON client for the Foxprox daemon.
//
...
//
// The wire DTOs are re-exported from foxprox/transport/httpjson so
...
// Client talks to an Foxprox daemon over HTTP.
type Client struct {
	base string
	http *http.Client
}
...
func (e *ErrHTTP) Error() string {
	return fmt.Sprintf("foxprox client: %s %s: %d %s", e.Method, e.Path, e.Status, e.Body)
}

// ForSocket builds a Client that dials the Unix socket at path. The synthetic
// http://foxprox base works because the custom DialContext ignores addresses.
...
func (c *Client) do(ctx context.Context, method, path string, body, out any) error {
	...
	if body != nil {
		...
		if err != nil {
			return fmt.Errorf("foxprox client: marshal %s %s: %w", method, path, err)
		}
		r = bytes.NewReader(raw)
	}
	req, err := http.NewRequestWithContext(ctx, method, c.base+path, r)
	if err != nil {
		return fmt.Errorf("foxprox client: build %s %s: %w", method, path, err)
	}
	if body != nil {
	...
	if err != nil {
		return fmt.Errorf("foxprox client: %s %s: %w", method, path, err)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(out); err != nil && !errors.Is(err, io.EOF) {
		return fmt.Errorf("foxprox client: decode %s %s: %w", method, path, err)
	}
	return nil
}
