...
This is useful for deciding which shell-like workflows are worth optimizing.`,
		Example: `  foxctl shell toolcalls
  foxctl shell toolcalls --source claude --transcript-limit 200
  foxctl shell toolcalls --save-file .foxctl/reports/transcript-toolcalls.json`,
		Args: cobra.NoArgs,
...
func runShellToolcallsCommand(cmd *cobra.Command, source, claudeDir, codexHome string, transcriptLimit, limit, commandLimit int, saveFile string) error {
	...
	if source != "all" && source != "claude" && source != "codex" {
		return protocol.WriteError(cmd.OutOrStdout(), "foxctl.shell.toolcalls", protocol.ErrorCodeEARG, "source must be one of: all, claude, codex", nil, protocol.WithSource("cli"))
	}

	acc, err := collectTranscriptAccumulator(source, claudeDir, codexHome, transcriptLimit)
	if err != nil {
		return protocol.WriteError(cmd.OutOrStdout(), "foxctl.shell.toolcalls", protocol.ErrorCodeERuntime, err.Error(), nil, protocol.WithSource("cli"))
	}
	...
	return protocol.Write(cmd.OutOrStdout(), protocol.OK("foxctl.shell.toolcalls", payload, protocol.WithSource("cli")))
}
