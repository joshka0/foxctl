...
4. Store tokens at ~/.foxctl/auth/claude_token.json

After login, set USE_CLAUDE_MAX=1 to use Claude for codemap generation.

Examples:
  foxctl auth claude-login
  foxctl auth claude-login --no-browser  # Manual URL copy`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				if os.IsNotExist(err) {
					fmt.Fprintln(cmd.OutOrStdout(), "Not logged in to Claude Max")
					fmt.Fprintln(cmd.OutOrStdout(), "Run: foxctl auth claude-login")
					return nil
				}
...
func runClaudeLogin(cmd *cobra.Command, noBrowser bool) error {
	...
	fmt.Fprintln(out, "  foxctl codemap generate \"your query\"")
...
func getClaudeTokenPath() (string, error) {
	...
	return filepath.Join(home, ".foxctl", "auth", "claude_token.json"), nil
}
...
<html>
<head>
    <title>Login Successful - foxctl</title>
    <style>
        body {
