...
Examples:
  ...
  foxctl todo list --status pending -f compact

  # Ranked by PageRank (most depended-upon first)
  foxctl todo list --ranked -f table

  # Pending tasks sorted by critical path
  foxctl todo list --status pending --sort-by critical_path

  # Extract just titles with jq
  foxctl todo list --jq '.data.tasks[].title'

  # Full metrics for all tasks
  foxctl todo list --ranked --include-metrics`,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
Examples:
  # Draft a plan
  foxctl todo plan --goal "Implement user authentication"

  # Apply a plan with scope paths
  foxctl todo plan --goal "Add OAuth2 support" --scope cmd/auth --scope internal/auth --apply

  # Refine an existing epic
  foxctl todo plan --goal "Add Google provider" --attach-to <epic-id> --apply`,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
Requires task embeddings to be generated first using:
  foxctl run embedding/tasks --input '{"scope": "all"}'

Examples:
  # Search for tasks related to authentication
  foxctl todo search "user authentication"

  # Search with a higher result limit
  foxctl todo search "database optimization" --limit 20

  # Search with a higher similarity threshold
  foxctl todo search "API endpoints" --min-similarity 0.5`,
...
func runTodoSkillWithFormat(cmd *cobra.Command, payload map[string]any, format, jq string) error {
	...
	if err != nil {
		return fmt.Errorf("todo/manage skill not found (run make skills-build or foxctl skills install): %w", err)
	}
