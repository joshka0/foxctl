	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			if len(sourceFiles) == 0 {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.consolidate_doctrine_group", "--source-file is required", "Pass one or more --source-file values.")
			}
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				...
				return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.consolidate_doctrine_group", payload)
			})
		},
	}
