...
func newSessionsDeriveMemoryCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.derive_memory", "--memory-lane must be one of: insight, doctrine, mixed", "Use --memory-lane insight for decision support, doctrine for doctrine-first extraction, or mixed for the legacy full pipeline.")
			}
			if lane == transcriptMemoryLaneInsight && persistMemory {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.derive_memory", "--persist-memory is not supported with --memory-lane insight", "Use the insight lane for decision support output only, or switch to doctrine/mixed if you want persistence.")
			}
			if lane != transcriptMemoryLaneInsight && persistHistory {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.derive_memory", "--persist-history is only supported with --memory-lane insight", "Use --memory-lane insight when persisting history-oriented retrieval records.")
			}
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				...
				default:
					return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.derive_memory", "--memory-lane must be one of: doctrine, mixed", "Use --memory-lane doctrine for the doctrine-first path or --memory-lane mixed for the legacy full pipeline.")
				}
				if err != nil {
					return err
				}
				...
				return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.derive_memory", payload)
			})
		},
	}
