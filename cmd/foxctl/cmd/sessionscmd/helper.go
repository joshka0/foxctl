...
func WriteNotFound(out io.Writer, command, sessionID string) error {
	data := map[string]any{
		...
		"hint":       fmt.Sprintf("No session with ID %q exists. Use 'foxctl sessions list' to see available sessions.", sessionID),
	}
	env := protocol.Error(
		command,
...
func FindUncapturedSessions(projectsDir string, captured map[string]bool, projectFilter string) ([]UncapturedSession, error) {
	...
	for _, entry := range entries {
		...
		// Parse workspace path from directory name (e.g., -Users-jkatigbak-repos-personal-foxctl)
