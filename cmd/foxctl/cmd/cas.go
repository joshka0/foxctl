...
func newCASPutCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.put", obj, protocol.WithSource("run"))
		},
	}
...
func newCASHeadCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.head", obj, protocol.WithSource("run"))
		},
	}
	return cmd
}
...
func newCASGetCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.get", data, protocol.WithSource("run"))
		},
	}
...
Examples:
  foxctl cas read sha256:abc123              # Read first page
  foxctl cas read sha256:abc123 --page 2     # Read second page
  foxctl cas read sha256:abc123 --page-size 4096  # Larger pages`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if pageSize <= 0 {
			...
			if kind != "" && !(strings.HasPrefix(kind, "text/") || strings.Contains(kind, "utf-8") || strings.Contains(kind, "json")) {
				return fmt.Errorf("cas: object kind %q is not text; use `foxctl cas get` to download binary content", obj.Kind)
			}
			...
			if remaining > 0 {
				...
				buf = buf[:n]
				if !utf8.Valid(buf) {
					return fmt.Errorf("cas: content is not valid UTF-8; use `foxctl cas get` for binary data")
				}
			...
			if page < totalPages {
				...
				data["next_command"] = fmt.Sprintf("foxctl cas read %s --page %d", args[0], page+1)
			}
			if page > 1 {
				data["prev_page"] = page - 1
			}

			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.read", data, protocol.WithSource("run"))
		},
	}
...
func newCASListCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.list", map[string]any{"objects": objects}, protocol.WithSource("run"))
		},
	}
	return cmd
}
...
func newCASPinCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.pin", map[string]string{"digest": args[0]}, protocol.WithSource("run"))
		},
	}
	return cmd
}
...
func newCASUnpinCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.unpin", map[string]string{"digest": args[0]}, protocol.WithSource("run"))
		},
	}
	return cmd
}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dryRun {
				...
				return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.rm", result, protocol.WithSource("run"))
			}
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.rm", map[string]string{"digest": args[0]}, protocol.WithSource("run"))
		},
	}
...
func newCASGCCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.cas.gc", data, protocol.WithSource("run"))
		},
	}
