...
func writeRunValidationError(cmd *cobra.Command, skill string, cause error) error {
	...
	env := protocol.ValidationError("foxctl.run", msg, data, protocol.WithSource("cli"))
...
func writeTimeoutError(cmd *cobra.Command, skill string, timeout time.Duration) error {
	...
	data := map[string]any{
		...
		"hint":    "Increase timeout with --timeout flag (e.g., --timeout 5m) or check if there are stuck jobs with: sqlite3 ~/.foxctl/jobs/jobs.db \"SELECT id,command,state FROM jobs WHERE state='running'\"",
	}

	env := protocol.Error("foxctl.run", protocol.ErrorCodeETimeout, msg, data, protocol.WithSource("cli"))
	if err := protocol.Write(cmd.OutOrStdout(), env); err != nil {
