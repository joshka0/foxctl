...
func TestPromptOptimizerBackendLabel(t *testing.T) {
	...
	if got := promptOptimizerBackendLabel("copro", "auto"); got != "foxctl" {
		t.Fatalf("copro auto backend=%q want foxctl", got)
	}
	if got := promptOptimizerBackendLabel("gepa", "foxctl"); got != "foxctl" {
		t.Fatalf("explicit backend=%q want foxctl", got)
	}
}
