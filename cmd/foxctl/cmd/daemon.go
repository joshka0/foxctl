...
func newDaemonCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "daemon",
		Short: "Run foxctl as a persistent daemon for faster hook execution",
		Long: `Run foxctl as a persistent daemon for faster hook execution.
...
Usage in hooks:
  ...
  if foxctl daemon status --quiet; then
    foxctl run hooks/task_guard --daemon --input "$input"
  else
    foxctl run hooks/task_guard --ephemeral --input "$input"
  fi`,
	}
	cmd.AddCommand(
...
func newDaemonStartCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "start",
		Short: "Start the foxctl daemon",
		Long: `Start the foxctl daemon for faster skill execution.
...
Examples:
  foxctl daemon start
  foxctl daemon start --background
  foxctl daemon start --workspace /path/to/project`,
...
func newDaemonStopCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `Stop the running foxctl daemon gracefully.
...
func newDaemonStatusCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Check if the foxctl daemon is running.

Returns exit code 0 if running, 1 if not.
Use --quiet to suppress output (for shell scripts).

Examples:
  foxctl daemon status
  foxctl daemon status --quiet && echo "Running"`,
		RunE: func(cmd *cobra.Command, _ []string) error {
