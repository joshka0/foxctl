...
func newObsidianGraphBuildCommand() *cobra.Command {
	...
	cmd.Flags().StringVar(&folder, "folder", "", "Vault folder override (default: inbox/drafted-from-foxctl/repo-graph/<project>)")
...
func newObsidianGraphPromoteCommand() *cobra.Command {
	...
	cmd.Flags().StringVar(&workspacePath, "workspace", "", "Workspace path (default: auto-detect from cwd)")
	cmd.Flags().StringVar(&project, "project", "", "Project name override")
	cmd.Flags().StringVar(&sourceFolder, "source-folder", "", "Draft bundle folder inside the vault (default: inbox/drafted-from-foxctl/repo-graph/<project>)")
