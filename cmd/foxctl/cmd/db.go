...
import (
	...
	"github.com/joshka0/foxctl/internal/agent/daemon"
...
Examples:
  # Migrate all stores
  foxctl db migrate --target-dsn "postgres://foxctl:dev@localhost:5432/foxctl?sslmode=disable"

  # Migrate only memory and tasks
  foxctl db migrate --target-dsn "postgres://..." --stores memory,tasks

  # Drop and recreate target tables before migrating
  foxctl db migrate --target-dsn "postgres://..." --drop

  # Preview migration without persisting changes
  foxctl db migrate --target-dsn "postgres://..." --dry-run`,
		RunE: func(cmd *cobra.Command, _ []string) error {
