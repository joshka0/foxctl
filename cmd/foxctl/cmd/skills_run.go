...
Examples:
  ...
  foxctl skills run code/complexity --path . --analysis-mode hotspots

  # Using JSON input
  foxctl skills run code/complexity --input '{"path":".", "analysis_mode":"hotspots"}'

  # Mixed: JSON base with flag overrides
  foxctl skills run code/complexity --input '{"path":"."}' --analysis-mode hotspots

  # Show available parameters for a skill
  foxctl skills run code/complexity --params-help
`,
...
func showParameterHelp(cmd *cobra.Command, manifest skill.Manifest) error {
	...
	if manifest.Signature.Help != nil && len(manifest.Signature.Help.Workflows) > 0 {
		...
		if wf.ExampleInput != nil {
			...
			fmt.Fprintf(w, "  foxctl skills run %s --input '%s'\n", manifest.Signature.Command, exampleJSON)
		}
	}
