...
The GEPA mode is a local GEPA-style reflective optimizer built on foxctl's
...
Examples:
  ...
  foxctl optimize session analyze

  # Analyze feedback from the last week
  foxctl optimize session analyze --since 2024-01-01

  # Only analyze sessions with low ratings
  foxctl optimize session analyze --max-rating 2

  # Focus on failed sessions
  foxctl optimize session analyze --outcome failure`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			out := cmd.OutOrStdout()
