...
import (
	...
	"github.com/joshka0/foxctl/internal/storage/console"
...
This launches the foxctl viewer in console mode, connected to the specified
...
Examples:
  ...
  foxctl console attach --actor my-coder --session 01JFABC...`,
...
Examples:
  ...
  foxctl console list --workspace /path/to/project`,
	RunE: runConsoleList,
}
...
func runConsoleAttach(cmd *cobra.Command, _ []string) error {
	...
	data := map[string]any{
		...
		"message":    fmt.Sprintf("Console ready. Launch: foxctl viewer --actor-console %s --console %s", consoleActorID, consoleID),
	}
