		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if inputPath == "-" {
				...
				if isTerminalReader(reader) {
					...
					if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, "stdin is a terminal", payload, opts()...); writeErr != nil {
				...
				if err != nil {
					...
					if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, fmt.Sprintf("open input: %v", err), payload, opts()...); writeErr != nil {
						return fmt.Errorf("write envelope: %w", writeErr)
					}
			...
			if err != nil {
				...
				if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, fmt.Sprintf("read input: %v", err), payload, opts()...); writeErr != nil {
					return fmt.Errorf("write envelope: %w", writeErr)
				}
			...
			if len(bytes.TrimSpace(raw)) == 0 {
				...
				if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, "no input provided", payload, opts()...); writeErr != nil {
					return fmt.Errorf("write envelope: %w", writeErr)
				}
			...
			if err := dec.Decode(&env); err != nil {
				...
				if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, fmt.Sprintf("decode envelope: %v", err), payload, opts()...); writeErr != nil {
					return fmt.Errorf("write envelope: %w", writeErr)
				}
			...
			if strict {
				...
				if consumed < len(raw) {
					...
					if len(trailing) > 0 {
						...
						if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, "unexpected trailing data after envelope", payload, opts()...); writeErr != nil {
							return fmt.Errorf("write envelope: %w", writeErr)
						}
			...
			if err := protocol.Validate(env); err != nil {
				...
				if writeErr := protocol.WriteError(cmd.OutOrStdout(), "foxctl.proto.validate", protocol.ErrorCodeEEnvelope, "protocol validation failed", payload, opts()...); writeErr != nil {
					return fmt.Errorf("write envelope: %w", writeErr)
				}
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.proto.validate", payload, opts()...)
		},
	}
