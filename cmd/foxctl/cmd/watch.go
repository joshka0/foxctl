...
import (
	...
	testwatchrunner "github.com/joshka0/foxctl/internal/tooling/testwatch"
...
Configuration is read from .foxctl/test-watch.yaml in the workspace root.
Use 'foxctl test-watch add' to configure watchers.
...
Modes:
  ...
  - --status-only: Print current test status and exit`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if err != nil {
				return fmt.Errorf("load foxctl config: %w", err)
			}
			...
			if !exists {
				return fmt.Errorf("no test-watch configuration found at %s\n\nUse 'foxctl test-watch add' to configure watchers", testwatch.ConfigPath(workspaceDir))
			}

			if len(twCfg.Watchers) == 0 {
				return fmt.Errorf("no watchers configured in %s\n\nUse 'foxctl test-watch add' to add watchers", testwatch.ConfigPath(workspaceDir))
			}
