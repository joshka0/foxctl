	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			data := map[string]any{
				...
				"hint": "Check store configs and network connectivity, then retry with: foxctl sync",
			}

			if failures > 0 {
				_ = protocol.WriteError(cmd.OutOrStdout(), "foxctl.sync", protocol.ErrorCodeERuntime, "sync failed for one or more stores", data, protocol.WithSource("cli"))
				cmd.SilenceErrors = true
				return fmt.Errorf("sync failed for %d store(s)", failures)
			}
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.sync", data, protocol.WithSource("cli"))
		},
	}
