...
import (
	...
	backupDomain "github.com/joshka0/foxctl/internal/domain/backup"
	"github.com/joshka0/foxctl/internal/platform/config"
	"github.com/joshka0/foxctl/internal/protocol"
	"github.com/joshka0/foxctl/internal/storage/backup"
...
func newBackupCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "backup",
		Short: "Backup and restore foxctl data",
		Long: `Backup and restore foxctl databases, CAS, memory, sessions, and jobs.
...
func newBackupCreateCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Create a backup archive containing foxctl data.
...
Examples:
  ...
  foxctl backup create --name "before-migration"

  # Backup only databases
  foxctl backup create --components databases

  # Backup everything except jobs
  foxctl backup create --exclude jobs

  # Backup to a specific location
  foxctl backup create --output /path/to/backup.tar.gz`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.backup.create", data, protocol.WithSource("run"))
		},
	}
...
func newBackupListCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.backup.list", data, protocol.WithSource("run"))
		},
	}
...
func newBackupInfoCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.backup.info", data, protocol.WithSource("run"))
		},
	}
...
func newBackupRestoreCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Restore foxctl data from a backup archive.
...
Examples:
  ...
  foxctl backup restore backup_2024-01-15_120000.tar.gz

  # Restore only databases
  foxctl backup restore backup.tar.gz --components databases

  # Force overwrite existing files
  foxctl backup restore backup.tar.gz --force

  # Dry run to see what would be restored
  foxctl backup restore backup.tar.gz --dry-run`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, ok := config.FromContext(cmd.Context())
			if !ok {
			...
			return protocol.WriteOK(cmd.OutOrStdout(), "foxctl.backup.restore", data, protocol.WithSource("run"))
		},
	}
