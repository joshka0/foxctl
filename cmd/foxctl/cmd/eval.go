...
import (
	...
	"github.com/joshka0/foxctl/internal/tooling/evals/correctioneval"
	"github.com/joshka0/foxctl/internal/tooling/evals/retrievaleval"
	"github.com/joshka0/foxctl/internal/tooling/evals/transcriptmemoryeval"
...
func newEvalCommand() *cobra.Command {
	...
	cmd.AddCommand(newEvalLongmemCommand())
	return cmd
}
...
func newEvalTranscriptMemoryExperimentCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if resolvedSaveDir == "" {
				resolvedSaveDir = filepath.Join(resolveContextWorkspace(""), ".foxctl", "exports", "evals", "transcript-memory")
			}
			resolvedLogFile := strings.TrimSpace(logFile)
			if resolvedLogFile == "" {
				resolvedLogFile = filepath.Join(resolveContextWorkspace(""), ".foxctl", "runtime", "transcript_memory_experiments.ndjson")
			}
...
func newEvalRetrievalCommand() *cobra.Command {
	...
	cmd.Flags().StringVar(&suiteRef, "suite", "", "Suite name (foxctl, praze) or explicit YAML path")
	...
	cmd.Flags().StringVar(&saveDir, "save-dir", "", "Override save directory (default: <workspace>/.foxctl/exports/evals)")
	return cmd
}
...
func runRepoIndexDAGEvalMode(ctx context.Context, storageRoot, workspacePath, query string, limit int, includeSemanticAnchors bool) ([]string, error) {
	store, err := repoindex.Open(ctx, storageRoot, resolveContextWorkspace(workspacePath))
	if err != nil {
		return nil, err
	}
...
func saveEvalOutputs(workspacePath, saveDir, suite string, result retrievaleval.RunResult, markdown string) (string, string, error) {
	...
	if dir == "" {
		dir = filepath.Join(resolveContextWorkspace(workspacePath), ".foxctl", "exports", "evals")
	}
