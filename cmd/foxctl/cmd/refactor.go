...
import (
	...
	refchanges "github.com/joshka0/foxctl/internal/intelligence/refactor/changes"
	refdeps "github.com/joshka0/foxctl/internal/intelligence/refactor/deps"
...
Common flag patterns:
  ...
  foxctl refactor scout --path ./internal --language go --focus slop --view grouped

  # Review DB and persistence anti-pattern candidates on an Ecto scope
  foxctl refactor scout --path apps/praze-api/lib --language elixir --focus slop --view grouped

  # Ask the advisor to rank a shortlist from the local scout
  foxctl refactor advisor --path ./internal --language go --focus slop --shortlist-size 5`,
	}
	cmd.AddCommand(
...
graph evidence, dead-code analysis, and dependency expansion will be available.`,
		Example: `  foxctl refactor status --path ./internal --language go
  foxctl refactor status --path apps/praze-api/lib --language elixir --include-tests`,
		Args: cobra.NoArgs,
...
when you want a stable reference point.`,
		Example: `  foxctl refactor snapshot --path ./internal --language go
  foxctl refactor snapshot --path apps/praze-api/lib --language elixir --include-tests`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
workspace.`,
  ...
  foxctl refactor deps --path ./internal --language go --query handleAsk --depth 2

  # Expand incoming dependencies from an explicit seed
  foxctl refactor deps --path ./internal --language go --seed symbol:go:internal/runtime/actor/agent_actor.go:*AgentActor.handleAsk --direction in`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
scout or reviewing evidence artifacts.`,
		Example: `  foxctl refactor changes --path ./internal --language go --since HEAD~10
  foxctl refactor changes --path ./internal --language go --since refsnap-123456`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
combine with structural scout findings.`,
		Example: `  foxctl refactor hot --path ./internal --language go
  foxctl refactor hot --path apps/praze-api/lib --language elixir --since HEAD~50 --max-results 30`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
context again without rerunning the analysis.`,
		Example: `  foxctl refactor evidence --snapshot-id refsnap-123456
  foxctl refactor evidence --artifact sha256:abcd... --full`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
For skill-targeted review:
  ...
  foxctl refactor scout --path ./internal/runtime/actor --language go --focus dead --view raw --min-score 0`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
...
Most important knobs:
  ...
  foxctl refactor advisor --path apps/praze-api/lib --language elixir --focus slop --max-findings 10 --shortlist-size 5

  # Override the second-stage model
  foxctl refactor advisor --path ./internal --language go --provider openrouter --model google/gemini-3.1-flash-lite-preview`,
		Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			input := map[string]any{
				"path":           path,
...
func runRefactorDeps(cmd *cobra.Command, workspace, path, language string, includeTests bool, seeds []string, query string, seedLimit int, edgeSets, edgeTypes []string, direction string, depth, budget, perNodeCap int) error {
	...
	if !status.RepoIndex.Available {
		hint := fmt.Sprintf("Build the repo index first, for example: foxctl index repo build --workspace %s", scope.Workspace)
