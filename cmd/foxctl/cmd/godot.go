...
Common workflows:
  foxctl godot ping                           # Check connection
  foxctl godot scene-tree                     # Get scene hierarchy
  foxctl godot inspect /root/Main/Player      # Inspect a node
  foxctl godot create /root/Main Node2D Enemy # Create a node
  foxctl godot set /root/Main/Player position "Vector2(100, 200)"
...
func newGodotSceneTreeCommand() *cobra.Command {
  ...
  foxctl godot scene-tree

  # Limit depth and nodes
  foxctl godot scene-tree --max-depth 3 --max-nodes 50`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			payload := map[string]any{
				"action":     "scene_tree",
				"host":       f.host,
...
func newGodotInspectCommand() *cobra.Command {
  ...
  foxctl godot inspect /root/Main/Player

  # Inspect root node
  foxctl godot inspect /root`,
...
The operation is registered with Godot's Undo/Redo system.`,
  ...
  foxctl godot create /root/Main Node2D Enemy

  # Create a CharacterBody2D
  foxctl godot create /root/Main CharacterBody2D Player`,
...
The operation is registered with Godot's Undo/Redo system.`,
  ...
  # Set visibility
  foxctl godot set /root/Main/Player visible false

  # Set modulate color
  foxctl godot set /root/Main/Player modulate "#ff0000"`,
		Args: cobra.ExactArgs(3),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
...
The operation is registered with Godot's Undo/Redo system.`,
  ...
  foxctl godot attach-script /root/Main/Player res://scripts/player.gd`,
...
func newGodotConnectSignalCommand() *cobra.Command {
  ...
  foxctl godot connect-signal /root/Main/Button pressed /root/Main/Player _on_button_pressed

  # Connect area entered signal
  foxctl godot connect-signal /root/Main/Area2D body_entered /root/Main/Player _on_area_entered`,
...
func newGodotDeleteCommand() *cobra.Command {
  ...
  foxctl godot delete /root/GameRoot/TestNode`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":     "node_delete",
...
func newGodotRenameCommand() *cobra.Command {
  ...
  foxctl godot rename /root/GameRoot/OldName NewName`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":     "node_rename",
...
func newGodotReparentCommand() *cobra.Command {
  ...
  foxctl godot reparent /root/GameRoot/Player /root/GameRoot/Entities`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
...
func newGodotClassInfoCommand() *cobra.Command {
  ...
  foxctl godot class-info CharacterBody2D

  # Get info about Node2D
  foxctl godot class-info Node2D`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":     "class_info",
...
All operations are registered with Godot's Undo/Redo system.`,
  ...
  foxctl godot ensure-node /root/GameRoot/Player Node2D

  # Ensure with properties
  foxctl godot ensure-node /root/GameRoot/Player CharacterBody2D \
    --prop position="Vector2(100, 200)" \
    --prop visible=true

  # Fail if node already exists
  foxctl godot ensure-node /root/GameRoot/NewNode Sprite2D --if-exists error`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newGodotSceneListCommand() *cobra.Command {
  ...
  foxctl godot scene-list

  # List scenes in a specific directory
  foxctl godot scene-list --path res://Scenes

  # Non-recursive listing
  foxctl godot scene-list --path res://Scenes --no-recursive`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			payload := map[string]any{
				"action":        "scene_list",
				"host":          f.host,
...
func newGodotSceneOpenCommand() *cobra.Command {
  ...
  foxctl godot scene-open res://Scenes/Main.tscn

  # Open a specific scene
  foxctl godot scene-open res://Scenes/Maps/Level1.tscn`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":     "scene_open",
...
The operation is registered with Godot's Undo/Redo system.`,
  ...
  foxctl godot scene-instance res://Scenes/Entities/Enemy.tscn /root/GameRoot

  # Instance with a custom name
  foxctl godot scene-instance res://Scenes/Entities/Enemy.tscn /root/GameRoot --name Boss`,
		Args: cobra.ExactArgs(2),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
...
Name supports * wildcard patterns.`,
  ...
  foxctl godot search-nodes --name "*Player*"

  # Find all Area2D nodes
  foxctl godot search-nodes --type Area2D

  # Find nodes with a specific property value
  foxctl godot search-nodes --property visible --value false

  # Combine filters
  foxctl godot search-nodes --type Sprite2D --property modulate --value "Color(1, 0, 0, 1)"`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			payload := map[string]any{
				"action":      "search_nodes",
				"host":        f.host,
...
func newGodotFocusCommand() *cobra.Command {
  ...
  foxctl godot focus /root/GameRoot/Player

  # Focus without framing
  foxctl godot focus /root/GameRoot/Player --no-frame`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":     "focus_node",
...
func newGodotSelectionCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  foxctl godot selection`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			payload := map[string]any{
...
func newGodotCameraSaveCommand() *cobra.Command {
  ...
  foxctl godot camera-save spawn_area

  # Save another position
  foxctl godot camera-save boss_arena`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":        "camera_save",
...
func newGodotCameraRestoreCommand() *cobra.Command {
  ...
  foxctl godot camera-restore spawn_area`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
...
func newGodotCameraListCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  foxctl godot camera-list`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			payload := map[string]any{
				"action":     "camera_list",
				"host":       f.host,
...
The path should be relative to res:// (e.g., Scripts/Player.gd).`,
  ...
  foxctl godot script-create res://Scripts/Player.gd --extends CharacterBody2D

  # Create with exports and methods
  foxctl godot script-create res://Scripts/Enemy.gd \
  ...
  # Create with signals
  foxctl godot script-create res://Scripts/GameManager.gd \
    --extends Node \
    --signal game_started \
    --signal game_over`,
		Args: cobra.ExactArgs(1),
...
func newGodotResourcesCommand() *cobra.Command {
  ...
  foxctl godot resources

  # List Scripts directory
  foxctl godot resources --path res://Scripts

  # List only .gd files
  foxctl godot resources --pattern "*.gd"`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			payload := map[string]any{
...
Supported type shortcuts:
  ...
  foxctl godot search-resources --type scene

  # Find all scripts with "Player" in the name
  foxctl godot search-resources --type script --name "*Player*"

  # Find textures in a specific directory
  foxctl godot search-resources --type texture --path res://Assets/Sprites`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if resourceType == "" {
...
Useful for understanding dependencies before refactoring or deleting resources.`,
  ...
  foxctl godot resource-references res://Scripts/Player.gd

  # Find what uses a texture
  foxctl godot resource-references res://Assets/player.png`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
...
func newGodotRunSceneCommand() *cobra.Command {
  ...
  foxctl godot run-scene res://Scenes/TestLevel.tscn

  # Run the main menu
  foxctl godot run-scene res://Scenes/MainMenu.tscn`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			payload := map[string]any{
				"action":     "run_scene",
...
func writeGodotSkillMissingError(cmd *cobra.Command) error {
	...
	hint := strings.Join([]string{
		...
		"  foxctl skills install --manifest skills/editor_godot/skill.yaml --binary dist/skills/editor_godot/bin",
	}, "\n")
