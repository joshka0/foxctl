...
Examples:
  # Search all scopes
  foxctl search "authentication flow"

  # Search only code symbols
  foxctl search "error handling" --scope symbols

  # Search with summarization (requires OpenRouter API key)
  foxctl search "database migrations" --summarize

  # Limit results per scope
  foxctl search "API endpoints" --limit 5`,
