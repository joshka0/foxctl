...
in foxctl's sessions.db. They can be summarized, searched, and used for
progressive memory retrieval.

Use the Stop hook (session-capture.sh) for automatic capture, or capture
manually with:
  foxctl run session/capture --input '{"workspace": "/path/to/project"}'`,
	}
	cmd.AddCommand(
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.list", payload)
				})
			})
		},
	}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				...
				if foundV2 {
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.show", v2Detail)
				}
				...
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if err != nil {
						if errors.Is(err, sessions.ErrNotFound) {
							return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.show", sessionID)
						}
						return err
					}
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.show", payload)
				})
			})
		},
	}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			if strings.TrimSpace(query) == "" {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.search",
					"--query is required",
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				...
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.search", payload)
				})
			})
		},
	}
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.stats", payload)
				})
			})
		},
	}
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if err != nil {
						if errors.Is(err, sessions.ErrNotFound) {
							return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.delete", sessionID)
						}
						return err
					}
					...
					if dryRun {
						...
						return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.delete", payload)
					}
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if dryRun {
						...
						return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.import", payload)
					}
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				...
				if dryRun {
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.resynthesize_v2", payload)
				}
...
Examples:
  foxctl sessions new
  foxctl sessions new --agent-id claude-code
  foxctl sessions new --parent 01HXYZ --agent-id my-agent
  foxctl sessions new --force  # close existing active session first`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					// Determine workspace
					...
					if active != nil {
						if force {
							...
							return sessionscmd.WriteConflict(cmd.OutOrStdout(), "foxctl.sessions.new",
								fmt.Sprintf("active session already exists: %s", active.ID),
								"Close it with 'foxctl sessions close' or use --force to close and create new")
						}
					}
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.new", payload)
				})
			})
		},
	}

	cmd.Flags().StringVar(&agentID, "agent-id", "", "Agent identifier (default: foxctl)")
...
Examples:
  foxctl sessions resume
  foxctl sessions resume --session 01HXYZ
  foxctl sessions resume --agent-id claude-code
  foxctl sessions resume --force`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					// Determine workspace
					...
					if active != nil {
						if force {
							...
							return sessionscmd.WriteConflict(cmd.OutOrStdout(), "foxctl.sessions.resume",
								fmt.Sprintf("active session already exists: %s", active.ID),
								"Close it with 'foxctl sessions close' or use --force to close and resume")
						}
					}
					...
					if sessionID != "" {
						...
						if err != nil {
							if errors.Is(err, sessions.ErrNotFound) {
								return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.resume", sessionID)
							}
							return err
						}
						parentSession = &s
						...
						if s == nil {
							payload := struct {
								...
								Hint:    "Create a new session with 'foxctl sessions new'",
							}
							return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.resume", payload)
						}
						parentSession = s
					}
...
Examples:
  foxctl sessions fork --parent 01HXYZ
  foxctl sessions fork --parent 01HXYZ --agent-id new-agent
  foxctl sessions fork --parent 01HXYZ --force`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if parentID == "" {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.fork",
					"--parent is required",
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if err != nil {
						if errors.Is(err, sessions.ErrNotFound) {
							return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.fork", parentID)
						}
						return err
					}
					...
					if active != nil {
						if force {
							...
							return sessionscmd.WriteConflict(cmd.OutOrStdout(), "foxctl.sessions.fork",
								fmt.Sprintf("active session already exists: %s", active.ID),
								"Close it with 'foxctl sessions close' or use --force to close and fork")
						}
					}
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.fork", payload)
				})
			})
		},
	}
...
Examples:
  foxctl sessions chain --session 01HXYZ
  foxctl sessions chain --session 01HXYZ --depth 10`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if sessionID == "" {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.chain",
					"--session is required",
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.chain", payload)
				})
			})
		},
	}
...
Examples:
  foxctl sessions close
  foxctl sessions close --status error
  foxctl sessions close --session 01HXYZ --status canceled`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			// Validate status
			switch status {
			case storage.SessionStatusOK, storage.SessionStatusError, storage.SessionStatusCanceled:
				// Valid
			default:
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.close",
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if sessionID != "" {
						...
						if active == nil {
							payload := struct {
								...
								Hint:    "Specify a session with --session or create one with 'foxctl sessions new'",
							}
							return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.close", payload)
						}
						targetID = active.ID
					...
					if err := store.SetStatus(ctx, targetID, status); err != nil {
						if errors.Is(err, sessions.ErrNotFound) {
							return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.close", targetID)
						}
						return err
					}
...
Examples:
  foxctl sessions export --session 01HXYZ
  foxctl sessions export --session 01HXYZ --output ./session.mv2
  foxctl sessions export --session 01HXYZ --include-turns --include-chunks
  foxctl sessions export --session 01HXYZ --dry-run`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if sessionID == "" {
				return sessionscmd.WriteArgError(cmd.OutOrStdout(), "foxctl.sessions.export",
					"--session is required",
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if err != nil {
						if errors.Is(err, sessions.ErrNotFound) {
							return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.export", sessionID)
						}
						return err
					}
					...
					if dryRun {
						...
						return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.export", payload)
					}
					...
					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.export", result)
				})
			})
		},
	}
...
Examples:
  foxctl sessions windows 01HXYZ...
  foxctl sessions windows 01HXYZ... --limit 5
  foxctl sessions windows 01HXYZ... --index 2 --full
  foxctl sessions windows 01HXYZ... --offset 5 --limit 5`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			sessionID := args[0]
			...
			return sessionscmd.WithConfig(cmd, func(ctx context.Context, cfg config.Config) error {
				return sessionscmd.WithSessionStore(ctx, cfg, func(store storage.SessionStore) error {
					...
					if err != nil {
						if errors.Is(err, sessions.ErrNotFound) {
							return sessionscmd.WriteNotFound(cmd.OutOrStdout(), "foxctl.sessions.windows", sessionID)
						}
						return err
					}
					...
					if showChunks && len(windows) > 0 {
						...
						if err == nil {
							...
							return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.windows", payloadWithChunks)
						}
					}

					return sessionscmd.WriteOK(cmd.OutOrStdout(), "foxctl.sessions.windows", payload)
				})
			})
		},
	}
