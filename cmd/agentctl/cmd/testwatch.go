...
Configuration is stored in .agentctl/test-watch.yaml in the workspace root.
This is agentctl-owned config (not harness-specific) and can be read by any
agent integration.

Use 'agentctl watch tests' to start the watcher daemon.`,
	}
	cmd.AddCommand(
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if !testwatch.ConfigExists(workspaceDir) {
				data := map[string]any{
					...
					"summary":       "No test-watch configuration found. Use 'agentctl test-watch add' to configure watchers.",
				}
...
Examples:
  ...
  agentctl test-watch add --id js --command "npm test" \
