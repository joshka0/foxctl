...
import (
	...
	envpkg "github.com/jkatigb/agentctl/internal/domain/envelope"
...
to raw shell output.`,
		Example: `  agentctl shell -- ls -la src/
  agentctl shell -- grep -rn "pub fn" src/
  agentctl shell -- git log --stat -10
  agentctl shell -- find internal -name "*.go"
  agentctl shell -- pytest -k unit tests/
  agentctl shell -- npm test --prefix packages/gui-agent
  agentctl shell --measure -- git diff --name-only
  agentctl bash -- go test -race ./...`,
...
func runShellCommand(cmd *cobra.Command, workspaceOverride, commandString string, measure bool, tokenModel string, args []string) error {
	...
	if err != nil {
		return protocol.WriteError(cmd.OutOrStdout(), "agentctl.shell", protocol.ErrorCodeEARG, err.Error(), map[string]any{
	...
	if err != nil {
		...
		return protocol.WriteError(cmd.OutOrStdout(), "agentctl.shell", code, err.Error(), data, protocol.WithSource("cli"), protocol.WithWorkspace(resolveShellWorkspace(workspaceOverride)))
	}
	...
	if routeErr != nil {
		return protocol.WriteError(cmd.OutOrStdout(), "agentctl.shell", protocol.ErrorCodeERuntime, routeErr.Error(), map[string]any{
			"input": map[string]any{
				"argv":      argv,
				"command":   shellreduce.JoinCommand(argv),
				"workspace": resolveShellWorkspace(workspaceOverride),
	...
	env := protocol.OK("agentctl.shell", payload, protocol.WithSource("cli"), protocol.WithWorkspace(resolveShellWorkspace(workspaceOverride)))
...
func executeStructuredShellRoute(cmd *cobra.Command, workspaceOverride string, route shellreduce.Route) (map[string]any, error) {
	...
	if err != nil {
		if writeErr := protocol.WriteError(cmd.OutOrStdout(), "agentctl.shell", protocol.ErrorCodeENotFound, err.Error(), map[string]any{
...
containing one command per line. Blank lines and # comments are ignored.`,
		Example: `  agentctl shell report \
    --command 'git log --stat -5' \
    --command 'find internal -name "*.go"'

  agentctl shell report --commands-file .agentctl/shell-bench.txt

  agentctl shell report --preset typical-bash

  agentctl shell report --preset transcript-derived

  agentctl shell report --preset typical-bash --save-file .agentctl/reports/shell-typical.json`,
		Args: cobra.NoArgs,
...
func runShellReportCommand(cmd *cobra.Command, workspaceOverride string, commands []string, commandsFile, preset, saveFile, transcriptSource string, transcriptLimit int, claudeDir, codexHome, tokenModel string) error {
	...
	if err != nil {
		return protocol.WriteError(cmd.OutOrStdout(), "agentctl.shell.report", protocol.ErrorCodeEARG, err.Error(), map[string]any{
	...
	if strings.TrimSpace(saveFile) != "" {
		if err := saveShellReport(resolveShellWorkspace(workspaceOverride), saveFile, payload); err != nil {
			return protocol.WriteError(cmd.OutOrStdout(), "agentctl.shell.report", protocol.ErrorCodeERuntime, err.Error(), nil, protocol.WithSource("cli"), protocol.WithWorkspace(resolveShellWorkspace(workspaceOverride)))
		}
		payload["saved_to"] = saveFile
	}
	return protocol.Write(cmd.OutOrStdout(), protocol.OK("agentctl.shell.report", payload, protocol.WithSource("cli"), protocol.WithWorkspace(resolveShellWorkspace(workspaceOverride))))
}
...
func shellReportPresetSpecs(preset, workspace, transcriptSource string, transcriptLimit int, claudeDir, codexHome string) ([]shellReportSpec, error) {
	...
	case "typical-bash":
		...
		specs := []shellReportSpec{
			...
			{Operation: "sed / nl", Command: "sed -n '1,120p' cmd/agentctl/cmd/shell.go", Weight: 8, Source: "preset:typical-bash"},
...
func transcriptDerivedReportSpecs(workspace string, acc *transcriptAccumulator) []shellReportSpec {
	...
	add("sed / nl", choose("sed / nl", "sed -n '1,120p' cmd/agentctl/cmd/shell.go"), weights["sed / nl"], false, "")
