...
// Run manually:
//
//	AGENTCTL_INTEGRATION_TMUX=1 go test -tags=integration ./cmd/agentctl/cmd/ -run IntegrationRelayRoomMessageTmuxConsumesInputRealTmux -v
func TestIntegrationRelayRoomMessageTmuxConsumesInputRealTmux(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			session := fmt.Sprintf("agentctl-relay-submit-%s-%d", strings.ReplaceAll(tc.name, "_", "-"), time.Now().UnixNano())
			tmpDir := t.TempDir()
...
// Run manually:
//
//	AGENTCTL_INTEGRATION_TMUX=1 go test -tags='integration libsqlite3' ./cmd/agentctl/cmd/ -run IntegrationRelayRoomMessageTmuxDispatchesQueuedDraftRealTmux -v
func TestIntegrationRelayRoomMessageTmuxDispatchesQueuedDraftRealTmux(t *testing.T) {
	...
	session := fmt.Sprintf("agentctl-relay-queued-%d", time.Now().UnixNano())
	tmpDir := t.TempDir()
