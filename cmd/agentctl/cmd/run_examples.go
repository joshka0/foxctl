	...
	examples := []runExample{
		{
			...
			Command:     `agentctl run fs/ls --input '{"path":"./src"}'`,
		...
		},
		{
			...
			Command:     "cat input.json | agentctl run text/grep --input-file -",
		},
	}

	payload := runExamplesPayload{
		Examples: examples,
		Hint:     "Pass a skill name to see examples for that skill (e.g., agentctl run todo/manage --examples).",
	}
	return protocol.WriteOK(cmd.OutOrStdout(), "agentctl.run.examples", payload, protocol.WithSource("cli"))
}
...
func formatRunExampleCommand(skillName string, input map[string]any) string {
	...
	return fmt.Sprintf("agentctl run %s --input '%s'", skillName, string(raw))
}
