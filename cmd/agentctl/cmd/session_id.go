...
Examples:
  agentctl session-id                           # Auto-detect from any provider
  agentctl session-id --provider claude         # Detect Claude Code session
  agentctl session-id --agent-id subagent:coder # For multi-agent coordination
  agentctl session-id --workspace /path         # Use specific workspace for Claude`,
...
func writeSessionIDSuccess(cmd *cobra.Command, sessionID, provider, workspace, agentID string) error {
	return sessionscmd.WriteOK(cmd.OutOrStdout(), "agentctl.session-id", map[string]any{
...
func writeSessionIDError(cmd *cobra.Command, message, detail string) error {
	return sessionscmd.WriteArgError(cmd.OutOrStdout(), "agentctl.session-id", message, detail)
}
