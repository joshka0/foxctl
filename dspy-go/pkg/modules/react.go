...
func NewReAct(signature core.Signature, registry *tools.InMemoryToolRegistry, maxIters int) *ReAct {
	...
	predict := NewPredict(modifiedSignature).WithStructuredOutput() // Prefer structured output for ReAct
...
func (r *ReAct) buildActionDescription() string {
	baseDescription := "The action to take. Use ONE of:\n" +
		"- XML: '<action><tool_name>...</tool_name><arguments><arg key=\"param_name\">value</arg></arguments></action>'\n" +
		"- JSON: '{\"tool_name\":\"tool\",\"arguments\":{\"param_name\":\"value\"}}'\n" +
		"To finish, use XML '<action><tool_name>Finish</tool_name></action>' or JSON '{\"tool_name\":\"Finish\"}'.\n" +
		"MUST INCLUDE and RETURN ONE ACTION at a time.\n\nAvailable tools:"
...
func (r *ReAct) parseActionString(ctx context.Context, actionStr string) (string, map[string]interface{}, error) {
	...
	// Try JSON action first for models that don't emit XML reliably.
	if strings.HasPrefix(strings.TrimSpace(actionStr), "{") {
		var jsonAction struct {
			ToolName  string                 `json:"tool_name"`
			Arguments map[string]interface{} `json:"arguments"`
		}
		if err := json.Unmarshal([]byte(actionStr), &jsonAction); err == nil && jsonAction.ToolName != "" {
			toolName := jsonAction.ToolName
			if strings.EqualFold(strings.TrimSpace(toolName), "finish") {
				toolName = "finish"
			}
			argsMap := jsonAction.Arguments
			if argsMap == nil {
