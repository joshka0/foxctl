...
func TestBuildFunctionSchemasWithArrayParameter(t *testing.T) {
	registry := tools.NewInMemoryToolRegistry()

	// Register a mock tool with array parameters (like code/context_ripgrep)
	mockTool := &mockTool{
		name:        "context_ripgrep",
		description: "Search for patterns and return surrounding code blocks",
		schema: models.InputSchema{
			Type: "object",
			Properties: map[string]models.ParameterSchema{
				"pattern": {
	...
	fn := functions[0]
	assert.Equal(t, "context_ripgrep", fn["name"])

	params := fn["parameters"].(map[string]interface{})
	properties := params["properties"].(map[string]interface{})

	// Verify string parameter doesn't have items
	patternProp := properties["pattern"].(map[string]interface{})
	assert.Equal(t, "string", patternProp["type"])
	_, hasItems := patternProp["items"]
	assert.False(t, hasItems, "string parameter should not have items field")

	// Verify array parameters have items field (JSON Schema requirement)
	globProp := properties["glob"].(map[string]interface{})
	assert.Equal(t, "array", globProp["type"])
	globItems, hasItems := globProp["items"].(map[string]interface{})
	assert.True(t, hasItems, "array parameter 'glob' must have items field for valid JSON Schema")
	assert.Equal(t, "string", globItems["type"], "array items should default to string type")

	globNotProp := properties["glob_not"].(map[string]interface{})
	assert.Equal(t, "array", globNotProp["type"])
	globNotItems, hasItems := globNotProp["items"].(map[string]interface{})
	assert.True(t, hasItems, "array parameter 'glob_not' must have items field for valid JSON Schema")
	assert.Equal(t, "string", globNotItems["type"], "array items should default to string type")
}
