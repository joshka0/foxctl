#!/usr/bin/env bash
# foxctl-hook.sh - Unified Claude Code hook adapter
#
...
dispatch_once() {
  ...
  if [[ $exit_code -ne 0 || -z "$resp" ]]; then
    if [[ "$HAS_ERROR_LIB" == "1" && -n "$stderr_content" ]]; then
      hook_error_block "foxctl-hook" "hooks/dispatch failed for $canonical_event" "$stderr_content"
    fi
  ...
  if [[ -n "$skill_error" && "$skill_error" != "null" ]]; then
    if [[ "$HAS_ERROR_LIB" == "1" ]]; then
      hook_error_block "foxctl-hook" "hooks/dispatch error for $canonical_event" "$skill_error"
    fi
    echo '{}'
    return 0
  fi
