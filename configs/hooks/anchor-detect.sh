#!/usr/bin/env bash
# Thin UserPromptSubmit anchor/todo prompt wrapper: delegate to Go.
...
if ! output="$(printf '%s' "$payload" | "$AGENTCTL_BIN" hooks anchor-detect --workspace "$workspace" 2>/dev/null)"; then
  echo '{"decision":"approve"}'
  exit 0
fi
