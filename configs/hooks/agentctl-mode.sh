#!/usr/bin/env bash
# agentctl-mode.sh - Consolidated UserPromptSubmit hook for agentctl mode
#
# Combines functionality from:
#   - agentctl-mode-detect.sh: Handle @strict on/off/status commands
#   - agentctl-mode-prompt.sh: Keyword-triggered actions (memory, search, recall, etc.)
#
# Usage in ~/.claude/settings.json:
#   "UserPromptSubmit": [
#     {
#       "matcher": "",
#       "hooks": ["$HOME/.claude/hooks/agentctl/agentctl-mode.sh"]
#     }
#   ]
#
# Commands:
#   @strict on     - Enable agentctl mode (redirect tools to agentctl skills)
#   @strict off    - Disable agentctl mode
#   @strict status - Show current agentctl mode status
#
# Keyword triggers:
...
if [[ "$prompt" =~ ^[[:space:]]*@(strict|agentctl) ]]; then
  ...
  # Parse command
  cmd="$(echo "$prompt" | sed -E 's/^[[:space:]]*@(strict|agentctl)[[:space:]]*//' | tr '[:upper:]' '[:lower:]' | xargs)"
...
- Bash sed/grep/find -> agentctl equivalents"
        else
...
Usage:
- \`@strict on\` - Enable agentctl mode
- \`@strict off\` - Disable agentctl mode
...
|| [[ "$prompt_lower" =~ (didn.?t\ (we|i)\ already|like\ before|as\ we\ discussed) ]]; then
  ...
  if [[ -n "$query" ]]; then
    ...
    if [[ -n "$mem_formatted" ]]; then
      ...
      context="**Recall:** No matching memories found for \"$query\". Try \`agentctl run session/recall\` for session history."
    fi
