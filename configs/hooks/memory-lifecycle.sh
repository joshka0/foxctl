#!/usr/bin/env bash
# Thin PostToolUse memory lifecycle wrapper: delegate to Go.
...
if ! output="$(printf '%s' "$payload" | "$AGENTCTL_BIN" hooks memory-lifecycle --workspace "$workspace" 2>/dev/null)"; then
  echo '{"decision":"approve"}'
  exit 0
fi
