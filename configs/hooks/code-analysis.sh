#!/usr/bin/env bash
# Thin PostToolUse code analysis wrapper: delegate to Go.

set -euo pipefail

if [[ "${FOXCTL_CODE_ANALYSIS_DISABLED:-}" == "1" ]]; then
  echo '{}'
  exit 0
fi
...
if ! output="$(printf '%s' "$payload" | "$FOXCTL_BIN" hooks code-analysis --workspace "$workspace" 2>/dev/null)"; then
  echo '{}'
  exit 0
fi
