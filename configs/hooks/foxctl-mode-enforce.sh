...
# When strict mode is enabled, it runs the corresponding foxctl skill and
...
# Check if foxctl mode is enabled (read from database)
enabled="false"
if [[ -f "$DB_PATH" ]]; then
...
# Self-protection: block edits to hook files and settings while foxctl mode is enabled
...
2. **Preview change**: \`foxctl run fs/apply_edit --input '{\"path\": \"$FILE_PATH\", \"edits\": [{\"search\": \"exact old text\", \"replace\": \"new text\"}], \"dry_run\": true}'\`
...
**To read specific lines:**
  \`foxctl run code/context_grep --input '{\"mode\": \"line\", \"file_path\": \"${FILE_PATH}\", \"line_start\": N, \"line_end\": M}'\`
...
**To read full file anyway (${LINE_COUNT} lines):**
  \`foxctl run fs/read --input '{\"path\": \"${FILE_PATH}\"}'\`"
  ...
  WebSearch|WebFetch)
    # Redirect web search/fetch to foxctl web tools for CAS limits and curated results
  ...
  Bash)
    ...
    # Block attempts to bypass foxctl mode via state manipulation
    ...
    # Block gh pr/api/checks commands - use foxctl ci instead
