#!/usr/bin/env bash
# Thin PostToolUse live index wrapper: delegate to Go.

set -euo pipefail

if [[ "${FOXCTL_LIVE_INDEX_DISABLED:-}" == "1" ]]; then
  echo '{}'
  exit 0
fi
...
if ! output="$(printf '%s' "$payload" | "$FOXCTL_BIN" hooks live-index --workspace "$workspace" 2>/dev/null)"; then
  echo '{}'
  exit 0
fi
