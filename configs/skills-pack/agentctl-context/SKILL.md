...
description: "ACA control plane, transcript-family history, context architecture, Obsidian knowledge-layer, and refactor scout/advisor workflows for agentctl."
---
...
- You are working on ACA or context architecture in `agentctl`.
...
```bash
agentctl sessions derive-memory --memory-lane insight --source-file <session.jsonl>
agentctl sessions derive-memory-group --memory-lane insight --source-file <a.jsonl> --source-file <b.jsonl>
```
...
```bash
agentctl capture --workspace . --task-id <id> --summary "..."
agentctl context promote --workspace . --path <handoff-or-observation>
```
