/**
 * agentctl CLI wrapper utilities for OpenCode plugins
 *
 * Provides typed interfaces for calling agentctl skills from TypeScript plugins.
 */

import { $ } from "bun";

/**
 * Result envelope from agentctl skill execution
 */
...
/**
 * Internal format for agentctl hooks/* skills (used by shell hooks)
 *
...
/**
 * Options for running agentctl skills
 */
...
/**
 * Run an agentctl skill and return typed result
 *
...
export async function runSkill<T = unknown>(
  ...
  try {
    const proc = Bun.spawn(["agentctl", ...args], {
      stdout: "pipe",
      stderr: "pipe",
    });
...
/**
 * Run an agentctl skill with input from a file
 *
