...
foxctl sessions list --workspace /path/to/project
foxctl sessions list --project myproject
```
...
```bash
foxctl sessions search "race condition mutex"
```
...
```bash
foxctl run session/recall --input '{"query": "database connection pool timeout"}'
```
...
```bash
foxctl run session/recall --input '{"query": "API authentication middleware"}'
```
...
```bash
foxctl run session/recall --input '{"query": "React hooks gotchas errors"}'
```
...
Sessions are stored in `~/.foxctl/storage/sessions.db`
