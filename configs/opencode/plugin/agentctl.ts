...
// Streamlined agentctl plugin - matches MCP skill groups
...
export const AgentctlPlugin: Plugin = async ({ $, directory }) => {
  const runSkill = async (skill: string, input: object): Promise<string> => {
    ...
    try {
      const result = await $`agentctl run ${skill} --input ${inputJson}`.text();
      return result;
    } catch (error: any) {
      return JSON.stringify({
        error: error.message || "agentctl skill failed",
      });
    }
  };
