...
```bash
foxctl run codemap/generate --input '{"query": "how does auth work", "depth": 1}' --timeout 10m
```
...
```bash
foxctl run code/semantic_search --input '{"query": "authentication", "scope": ["codemaps"]}'
```

## Import Codemaps

```bash
foxctl run codemap/import --input '{"path": "docs/codemaps", "recursive": false}'
```
...
Full docs: `~/.foxctl/share/configs/skills/foxctl-codemap/Skill.md`
